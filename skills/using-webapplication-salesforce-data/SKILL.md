...
| `bash skills/using-webapplication-salesforce-data/scripts/graphql-search.sh --schema ./schema.graphql <Entity>` | project root | Schema lookup against the current org schema |
...
|----------------|------------|
| `Cannot query field` | Field name is wrong — run `bash skills/using-webapplication-salesforce-data/scripts/graphql-search.sh --schema ./schema.graphql <Entity>` and use the exact name from the Type definition section |
